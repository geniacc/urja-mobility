        ...
        <motion.div layout className="products-grid" style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, minmax(0, 1fr))' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: isMobile ? '1.5rem' : '2.5rem', justifyContent: 'center' }}>
          <AnimatePresence mode="popLayout">
            {productsToDisplay.map(product => {
              ...
              return (
                <motion.div layout key={product.id} initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: hoveredProduct && hoveredProduct !== product.id ? 0.4 : 1, scale: hoveredProduct && hoveredProduct !== product.id ? 0.95 : 1, y: 0, filter: hoveredProduct && hoveredProduct !== product.id ? 'blur(2px)' : 'blur(0px)' }} exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }} whileHover={{ y: -12, scale: 1.02, zIndex: 10, transition: { duration: 0.2 } }} onHoverStart={() => setHoveredProduct(product.id)} onHoverEnd={() => setHoveredProduct(null)} style={{ background: 'var(--bg-2)', borderRadius: '16px', overflow: 'hidden', border: '1px solid var(--border)', position: 'relative', display: 'flex', flexDirection: 'column' }}>

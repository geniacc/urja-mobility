...
export default function Products() {
  ...
  // UPGRADED: Maps titles to the new "1.png" angle for the product grid thumbnail
  const getProductImage = (title) => {
    ...
    // Batteries (Using the "1.png" of your new 4-angle image sets)
  ...
  return (
    <div className="page-container" style={{ background: 'var(--bg)', minHeight: '100vh', overflowX: 'hidden', paddingTop: "var(--nav-height, 100px)" }}>
      ...
      {/* Hero Section */}
      <section className="products-hero-grid" style={{ position: 'relative', padding: heroPadding, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: heroGap, alignItems: 'center', minHeight: heroMinHeight }}>
        <motion.div className="products-hero-text" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
          <div style={{ display: 'inline-block', position: 'relative', color: accentColor, fontWeight: 700, letterSpacing: '0.5px', marginBottom: '1.5rem', fontSize: '1rem' }}>
            ...
            <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.3 }} style={{ position: 'absolute', left: 0, bottom: -6, height: 2, width: '100%', transformOrigin: '0%', background: accentColor, boxShadow: `0 0 12px ${accentColor}80` }} />
          </div>
          <h1 style={{ fontSize: 'clamp(3rem, 6vw, 5.5rem)', lineHeight: 1.1, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-1px', textShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
          ...
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }} style={{ fontSize: 'clamp(1.1rem, 2vw, 1.4rem)', color: 'var(--text-primary)', maxWidth: '700px', lineHeight: 1.8, marginBottom: '2.5rem', textShadow: '0 6px 20px rgba(0,0,0,0.35)' }}>
          ...
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
            {["General", "Electrical", "Mechanical", "Protection"].map((tag, i) => (
              <span key={i} style={{ border: "1px solid var(--border)", padding: "0.5rem 0.9rem", borderRadius: "999px", color: "var(--text-secondary)", background: "rgba(255,255,255,0.03)" }}>{tag}</span>
            ))}
          </div>
        </motion.div>

        <motion.div className="products-hero-visual" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} style={{ height: '500px', width: '100%', position: 'relative' }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '400px', height: '400px', background: `radial-gradient(circle, ${accentColor}20 0%, transparent 70%)`, filter: 'blur(40px)', zIndex: 0 }} />
      ...
      <section className="container" style={{ padding: isMobile ? "4rem 1.5rem 5rem" : "6rem 2rem" }}>

        {/* Category Tabs */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: isMobile ? '1.5rem' : '2rem', flexWrap: 'wrap', gap: isMobile ? '1rem' : '1.5rem' }}>
          {visibleCategories.map(cat => (
            <button className="product-category-button" key={cat.id} onClick={() => setActiveCategory(cat.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', padding: isMobile ? '0.75rem 1rem' : '1rem 1.5rem', borderRadius: '16px', border: '1px solid', borderColor: activeCategory === cat.id ? cat.color : 'var(--border)', background: activeCategory === cat.id ? `${cat.color}10` : 'var(--bg-2)', color: activeCategory === cat.id ? cat.color : 'var(--muted)', fontWeight: '600', cursor: 'pointer', transition: 'all 0.3s ease', minWidth: isMobile ? '110px' : '120px' }}>
              <span style={{ width: 46, height: 46, borderRadius: 14, display: "grid", placeItems: "center", border: `1px solid ${activeCategory === cat.id ? cat.color : "var(--border)"}`, background: activeCategory === cat.id ? `${cat.color}12` : "rgba(255,255,255,0.03)", color: activeCategory === cat.id ? cat.color : "var(--muted)" }}>
        ...
        {currentCategory?.subcategories.length > 0 && (
          <div className="product-subcategory-tabs" style={{ display: 'flex', justifyContent: 'center', marginBottom: currentSubcategory?.groups?.length > 0 ? (isMobile ? '0.75rem' : '1rem') : (isMobile ? '3rem' : '4rem'), flexWrap: 'wrap', gap: '0.5rem', padding: '0.5rem', background: 'var(--bg-2)', borderRadius: '99px', width: 'fit-content', margin: currentSubcategory?.groups?.length > 0 ? (isMobile ? '0 auto 0.75rem auto' : '0 auto 1rem auto') : (isMobile ? '0 auto 3rem auto' : '0 auto 4rem auto'), border: '1px solid var(--border)' }}>
            {currentCategory.subcategories.map(sub => (
              <button key={sub.id} onClick={() => setActiveSubcategory(sub.id)} style={{ padding: '0.5rem 1.5rem', borderRadius: '99px', border: 'none', background: activeSubcategory === sub.id ? accentColor : 'transparent', color: activeSubcategory === sub.id ? '#fff' : 'var(--muted)', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                {sub.title}
              </button>
            ))}
          </div>
        ...
        {currentSubcategory?.groups?.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: isMobile ? '2.5rem' : '4rem', flexWrap: 'wrap', gap: '0.5rem' }}>
            {currentSubcategory.groups.map(group => (
              <button key={group.id} onClick={() => setActiveGroup(group.id)} style={{ padding: '0.4rem 1.2rem', borderRadius: '99px', border: `1px solid ${activeGroup === group.id ? accentColor : 'var(--border)'}`, background: activeGroup === group.id ? accentColor : 'transparent', color: activeGroup === group.id ? '#fff' : 'var(--muted)', fontSize: '0.9rem', fontWeight: '500', cursor: 'pointer', transition: 'all 0.2s ease' }}>
                {group.title}
              </button>
            ))}
          </div>
        ...
        {/* Grid */}
        <motion.div layout className="products-grid" style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, minmax(0, 1fr))' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: isMobile ? '1.5rem' : '2.5rem', justifyContent: 'center' }}>
          <AnimatePresence mode="popLayout">
            {productsToDisplay.map(product => {
              ...
              const descriptionShort = isMobile ? (descriptionText.length > 110 ? descriptionText.slice(0, 110) + "..." : descriptionText) : descriptionText;
              ...
              return (
                <motion.div className="product-list-card" layout key={product.id} initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: hoveredProduct && hoveredProduct !== product.id ? 0.4 : 1, scale: hoveredProduct && hoveredProduct !== product.id ? 0.95 : 1, y: 0, filter: hoveredProduct && hoveredProduct !== product.id ? 'blur(2px)' : 'blur(0px)' }} exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }} whileHover={{ y: -12, scale: 1.02, zIndex: 10, transition: { duration: 0.2 } }} onHoverStart={() => setHoveredProduct(product.id)} onHoverEnd={() => setHoveredProduct(null)} style={{ background: 'var(--bg-2)', borderRadius: '16px', overflow: 'hidden', border: '1px solid var(--border)', position: 'relative', display: 'flex', flexDirection: 'column' }}>

                  {/* Card Header / Image Area */}
                  <div style={{ height: '200px', background: `linear-gradient(135deg, var(--bg-3) 0%, ${product.color}10 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', padding: '1.5rem' }}>
                    ...
                    <div style={{ position: 'absolute', bottom: '1rem', right: '1rem', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', padding: '0.25rem 0.75rem', borderRadius: '99px', fontSize: '0.75rem', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', zIndex: 2 }}>
                      {quickBadges[0] || "TBD"}
                    </div>

                    {isMobile && (
                      <button style={{ position: 'absolute', left: '1rem', bottom: '1rem', background: 'rgba(15,23,42,0.9)', borderRadius: '999px', border: `1px solid ${product.color}`, color: '#e5e7eb', padding: '0.4rem 0.9rem', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem', boxShadow: '0 10px 25px rgba(0,0,0,0.45)', zIndex: 21 }} onClick={(e) => { e.stopPropagation(); navigate(`/product/${product.id}`); }}>
                  ...
                  <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: '700', color: product.color, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.5rem' }}>{product.categoryName}</div>
                    <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '0.75rem', color: 'var(--text-primary)' }}>{product.title}</h3>
                    <p style={{ color: 'var(--muted)', fontSize: isMobile ? '0.9rem' : '0.95rem', lineHeight: 1.6, marginBottom: isMobile ? '1rem' : '1.5rem', flex: 1 }}>{descriptionShort}</p>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: isMobile ? "1rem" : "1.5rem" }}>
                      {(quickBadges.length ? quickBadges : ["TBD", "TBD"]).slice(0, 2).map((badge, idx) => (
                        <span key={`${product.id}-badge-${idx}`} style={{ fontSize: "0.75rem", padding: "0.35rem 0.7rem", borderRadius: "999px", border: "1px solid var(--border)", background: "rgba(255,255,255,0.03)", color: "var(--text-secondary)" }}>{badge}</span>
                      ))}
                    </div>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem" }}>
                <div style={{ background: "rgba(34,197,94,0.15)", padding: "0.4rem", borderRadius: "10px", display: "grid", placeItems: "center" }}>
                  <img src="/assets/zuice_logo.png" alt="Zuice Logo" style={{ width: "100px", height: "100px", objectFit: "contain" }} />
                </div>
                <span style={{ color: "#22c55e", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", fontSize: "0.85rem" }}>Partner Brand</span>
              </div>
              <h2 style={{ fontSize: "clamp(2rem, 4vw, 2.5rem)", fontWeight: 800, color: "#fff", marginBottom: "0.5rem", lineHeight: 1.1 }}>Zuice Inverters</h2>
              <p style={{ color: "var(--text-muted)", fontSize: "1.05rem", lineHeight: 1.6, maxWidth: "600px", margin: 0 }}>Looking for smart home and consumer-ready solar inverters? Explore the complete Zuice inverter lineup on our dedicated partner platform.</p>
            </div>
